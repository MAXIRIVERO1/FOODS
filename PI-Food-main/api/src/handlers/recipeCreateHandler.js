...
const recipeHandler = async(req, res)=>{
    try {
        ...
        if(title){const recipeCreate = await createRecipe({title, image, summary, healthScore, steps}, diet);//se incluye a diet asi porque el modelo Recipe no tiene un atributo diet
